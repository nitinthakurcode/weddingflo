...
Cluster S is fixed at the **application** layer (every site routes through `assertClientAccess`/`assertEntityAccess`/`withinCompanyClients`). The **DB-level RLS fail-closed backstop** — `app.current_company_id` + RLS policies on the tenant tables that lack an explicit `companyId` (so a future missing app-level check FAILS CLOSED instead of leaking) — is **NOT yet landed**. SCOPE CORRECTION: the "11 child tables" cited earlier (INVENTORY.md) was only the IDOR-sweep-**reachable** child subset; a full schema scan shows **~33+ tables in `schema-features.ts` alone (of ~98 tenant tables total) lack an explicit `companyId`** (parent-FK scoped only). So the 6B RLS backstop must cover the full set, not 11. The test DB role is superuser (RLS bypassed), so this phase proves the app-level scope, which is the prod reality for these tRPC paths today; RLS is defense-in-depth scheduled for the **next PR**. Until then, a NEW unscoped resolver would not be caught by the DB (only by the app-level convergence sweep + the CI Audit Gate).

**Status [6C]:** the **CI gate half** of the defense-in-depth (CONVERGENCE move #4) LANDED in PR #2 (`.github/workflows/audit.yml`, non-blocking first → promote to required once a real run is read green). RLS scaffolding EXISTS (18 tables carry `tenant_isolation` policies — migrations `0024` enable-all + `0027`/`0030`/`0031`; `with-tenant-scope.ts` + `ctx.withTenantScope` can set `app.current_company_id`) but is NOT load-bearing: resolvers use the raw `ctx.db` and `ctx.withTenantScope` is not applied by default, so RLS is unexercised/unverified. The **verified RLS fail-closed backstop** — extend policies to the ~33+ `companyId`-less tables, set the GUC per request, and add a **non-superuser CI role** that actually exercises RLS — is the **next PR**.
