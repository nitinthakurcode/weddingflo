...
export const activityFormSchema = z.object({
  ...
  activity_status: activityStatusSchema.default('pending'),
  start_time: z.string().regex(/^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/, 'Invalid time format (HH:MM)'),
  duration_minutes: z.number().positive('Duration must be positive').int().min(1).max(1440),
  vendor_ids: z.array(z.string()).default([]),
  location: z.string().max(200).optional(),
  assigned_to: z.array(z.string()).default([]),
  dependencies: z.array(activityDependencySchema).default([]),
  time_buffer_minutes: z.number().int().min(0).max(180).default(0),
