...
export default function BudgetPage() {
  ...
  // If there's exactly one client, redirect to their budget page
  useEffect(() => {
    if (clients && clients.length === 1) {
      router.push(`/dashboard/clients/${clients[0].id}/budget`);
    }
  }, [clients, router]);
      ...
      <div className="max-w-2xl w-full text-center space-y-6">
        ...
        <div className="space-y-2">
          ...
          <p className="text-lg text-muted-foreground">
            Budgets are managed per client
          </p>
        </div>

        {/* Description */}
        <p className="text-muted-foreground max-w-md mx-auto">
          To view and manage a wedding budget, please select a client from your client list.
          Each client has their own dedicated budget tracking.
        </p>
        ...
        <div className="pt-6 border-t max-w-md mx-auto">
          <p className="text-xs text-muted-foreground">
            <strong>Tip:</strong> You can also access budgets directly from the client detail page:
            Dashboard → Clients → [Select Client] → Budget
          </p>
        </div>
      </div>
    </div>
