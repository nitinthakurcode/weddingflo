...
export default function TimelinePage() {
  ...
  // If there's exactly one client, redirect to their timeline page
  useEffect(() => {
    if (clients && clients.length === 1) {
      router.push(`/dashboard/clients/${clients[0].id}/timeline`);
    }
  }, [clients, router]);
      ...
      <div className="flex-1 flex items-center justify-center p-4">
        <div className="text-center">
          ...
          <p className="text-muted-foreground mt-2">
            Please sign in to manage timelines.
          </p>
        </div>
      </div>
    );
      ...
      <div className="max-w-2xl w-full text-center space-y-6">
        ...
        <div className="space-y-2">
          ...
          <p className="text-lg text-muted-foreground">
            Timelines are managed per client
          </p>
        </div>

        {/* Description */}
        <p className="text-muted-foreground max-w-md mx-auto">
          To view and manage event timelines, please select a client from your client list.
          Each client has their own dedicated wedding timeline with tasks and milestones.
        </p>
        ...
        <div className="pt-6 border-t max-w-md mx-auto">
          <p className="text-xs text-muted-foreground">
            <strong>Tip:</strong> You can also access timelines directly from the client detail page:
            Dashboard → Clients → [Select Client] → Timeline
          </p>
        </div>
      </div>
    </div>
